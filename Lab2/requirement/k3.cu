...
__global__ void add_matrix(float*a,float*b,float*c,const int rows,const int cols){
    ...
    // Computing col Values
    // Conditon on only avaliable cols :D
    if(tid<cols){
        for(int i=0;i<rows;i++){
            c[tid+i*cols]=a[tid+i*cols]+b[tid+i*cols];
            printf("c=%f: a+b=%f+%f\n",c[tid+i*cols],a[tid+i*cols], b[tid+i*cols]);
        }
    }
...
int main(int argc, char* argv[]){
    ...
    while(test_cases_no>0){
        ...
        // Since we need one thread per col ceil(cols-1/256)
        int grid_size=((cols-1)/block_size)+1;

        //For the test case we have only 4 cols so only 1 block is very redundent 
